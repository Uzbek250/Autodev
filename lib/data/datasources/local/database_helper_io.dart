...
/// Mobile/desktop SQLite implementation. Kept unchanged for APK behavior.
class DatabaseHelper {
  ...
  Future<void> upsertProject(Map<String, dynamic> map) async {
    final db = await database;
    await db.insert('projects', map, conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<void> deleteProject(String id) async {
    ...
    await db.transaction((txn) async {
      ...
      await txn.delete('chat_history', where: 'project_id = ?', whereArgs: [id]);
    });
  }

  Future<List<Map<String, dynamic>>> getFilesForProject(String projectId) async {
    final db = await database;
    return db.query('files', where: 'project_id = ?', whereArgs: [projectId], orderBy: 'path ASC');
  }

  Future<void> upsertFile(Map<String, dynamic> map) async {
    final db = await database;
    await db.insert('files', map, conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<void> upsertFiles(List<Map<String, dynamic>> maps) async {
    ...
    await db.transaction((txn) async {
      for (final map in maps) {
        await txn.insert('files', map, conflictAlgorithm: ConflictAlgorithm.replace);
      }
    });
  }
  ...
  Future<List<Map<String, dynamic>>> getChatHistory(String projectId) async {
    final db = await database;
    return db.query('chat_history', where: 'project_id = ?', whereArgs: [projectId], orderBy: 'created_at ASC');
  }

  Future<void> insertChatMessage(Map<String, dynamic> map) async {
    final db = await database;
    await db.insert('chat_history', map, conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<void> upsertSettings(Map<String, dynamic> map) async {
    final db = await database;
    await db.insert('settings', {...map, 'id': 1}, conflictAlgorithm: ConflictAlgorithm.replace);
  }
}
