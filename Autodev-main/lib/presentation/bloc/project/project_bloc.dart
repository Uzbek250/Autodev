...
class ProjectBloc extends Bloc<ProjectEvent, ProjectState> {
  ...
  Future<void> _onPlanApproved(
    ...
    add(ProjectEngineerCompleteEvent(engineerResult.value!, updatedProject));
  }
  ...
  Future<void> _onEngineerComplete(
    ...
    // The project now arrives directly on the event (set when the engineer
    // run was kicked off), so this no longer depends on `state` still being
    // ProjectEngineerLoading — a stale/changed state used to silently fall
    // back to a placeholder project with id: '', hiding real bugs.
    final refreshed = await projectRepository.getProjectById(event.project.id);
    emit(ProjectEngineerComplete(
      files: event.files,
      project: refreshed ?? event.project,
    ));
  }
